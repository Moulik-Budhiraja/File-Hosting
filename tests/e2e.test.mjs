...
function singleFileTar(pathname, contents) {
  ...
  const text = (value, offset, length) =>
    header.write(value, offset, length, "ascii");
  const octal = (value, offset, length) =>
    text(`${value.toString(8).padStart(length - 1, "0")}\0`, offset, length);
  text(pathname, 0, 100);
  octal(0o644, 100, 8);
  octal(0, 108, 8);
  octal(0, 116, 8);
...
function parseJson(result, context) {
  ...
  try {
    ...
    assert.fail(
      `${context} returned invalid JSON: ${String(error)}\n${processDiagnostic(result)}`,
    );
  }
}
...
async function waitForServer(server, baseUrl) {
  ...
  while (Date.now() < deadline) {
    if (server.child.exitCode !== null) {
      throw new Error(
        `Next server exited before becoming healthy\n${server.logs()}`,
      );
    }
    try {
...
async function startServer({
  ...
  const child = spawn(
    process.execPath,
    [nextEntry, "start", "-H", "127.0.0.1", "-p", String(port)],
    {
      cwd: serverDir,
      env: {
        ...process.env,
...
test(
  "built server and CLI work together end to end",
  { timeout: 180_000 },
  async (t) => {
    ...
    const cli = (args, options = {}) =>
      runProcess(process.execPath, [cliEntry, ...args], {
    ...
    const uploadJson = async (args, options) => {
      ...
      assert(
        Array.isArray(body),
        `upload output should be an array: ${processDiagnostic(result)}`,
      );
    ...
    await t.test(
      "bootstrap login, custom API key, protected visibility, and revocation work",
      async () => {
        ...
        const activated = await fetch(
          `${baseUrl}/api/api-keys/${createdBody.api_key.id}/activate`,
          {
            method: "POST",
        ...
        assert.equal(
          (await request(`/raw/${uploaded.id}`, {}, false)).status,
          404,
        );
        assert.equal(
          (
            await fetch(`${baseUrl}/raw/${uploaded.id}`, {
              headers: { authorization: `Bearer ${apiKey}` },
            })
          ).status,
          200,
        ...
        const revoked = await fetch(
          `${baseUrl}/api/api-keys/${createdBody.api_key.id}`,
          {
            method: "DELETE",
            headers: { cookie, origin: baseUrl },
          },
    ...
    await t.test(
      "shorthand and explicit uploads preserve binary and empty data",
      async () => {
        persistenceBytes = Buffer.from([
          0x00, 0xff, 0x10, 0x80, 0x41, 0x00, 0x7f,
        ]);
        const binaryPath = path.join(fixtureDir, "binary.dat");
        const emptyPath = path.join(fixtureDir, "empty.txt");
        await Promise.all([
          writeFile(binaryPath, persistenceBytes),
        ...
        assert.deepEqual(
          Buffer.from(await binaryRaw.arrayBuffer()),
          persistenceBytes,
        );
    ...
    await t.test(
      ...
      async () => {
        ...
        const byQuery = parseJson(
          await cli(["find", "finance", "--json"]),
          "find by query",
        );
        assert.deepEqual(
          byQuery.map((item) => item.id),
          [report.id],
        );
        const byName = parseJson(
          await cli(["find", "--name", "report-202?.pdf", "--json"]),
          "find by glob name",
        );
        assert(byName.some((item) => item.id === report.id));
        assert(!byName.some((item) => item.id === notes.id));
        const byAllTags = parseJson(
          await cli([
            "find",
            "--tag",
            "finance",
        ...
        assert.deepEqual(
          nullResult.stdout.toString("utf8").split("\0").filter(Boolean).sort(),
          [notes.id, report.id].sort(),
        );
      },
    );

    await t.test(
      "public/private access and visibility transitions obey authentication",
      async () => {
        const publicPath = path.join(fixtureDir, "public.txt");
        const privatePath = path.join(fixtureDir, "private.txt");
        await Promise.all([
          writeFile(publicPath, "public"),
        ...
        for (const route of [`/${publicFile.id}`, `/raw/${publicFile.id}`]) {
          assert.equal(
            (await request(route, {}, false)).status,
            200,
        ...
        assert.deepEqual(
          Object.fromEntries(privatePage.headers),
          Object.fromEntries(missingPage.headers),
        );
        ...
        assert.doesNotMatch(
          privatePageBytes.toString("utf8"),
          new RegExp(`${privateFile.id}|private\\.txt`, "u"),
        );
        assert.equal(
          (await request(`/${privateFile.id}`)).status,
          200,
          "private preview should accept a token",
        );
        assert.equal(
          (await request(`/raw/${privateFile.id}`, {}, false)).status,
          404,
          "private raw bytes must remain hidden",
        );
        assert.equal(
          (await request(`/raw/${privateFile.id}`)).status,
          200,
          "private raw bytes should accept a token",
        );

        let changed = parseJson(
          await cli(["visibility", publicFile.id, "private", "--json"]),
          "make public file private",
        );
        assert.equal(changed.visibility, "private");
        assert.equal(
          (await request(`/raw/${publicFile.id}`, {}, false)).status,
          404,
        );
        changed = parseJson(
          await cli(["visibility", publicFile.id, "public", "--json"]),
    ...
    await t.test(
      "HTML and SVG previews escape source and set defensive headers",
      async () => {
        const htmlSource =
          '<script>globalThis.pwned=true</script><b title="x">hello & goodbye</b>';
        const svgSource =
        ...
        for (const [file, dangerous, escaped] of [
          ...
          assert(
            !page.includes(dangerous),
            "preview must not contain executable source markup",
          );
          assert(
            page.includes(escaped),
            "preview should render escaped source text",
          );
          assert.match(
          ...
          assert.equal(
            preview.headers.get("x-content-type-options"),
            "nosniff",
          );
    ...
    await t.test(
      "raw downloads implement exact single byte ranges",
      async () => {
    ...
    await t.test(
      "info returns canonical preview/raw URLs and tag mutations work",
      async () => {
        ...
        info = parseJson(
          await cli(["tag", file.id, "add", "reviewed", "alpha", "--json"]),
          "tag add",
        );
        assert.deepEqual([...info.tags].sort(), [
          "alpha",
          "initial",
          "reviewed",
        ]);
        info = parseJson(
          await cli(["tag", file.id, "remove", "initial", "--json"]),
          "tag remove",
        );
        assert.deepEqual([...info.tags].sort(), ["alpha", "reviewed"]);
        info = parseJson(
          await cli(["tag", file.id, "set", "final", "only", "--json"]),
          "tag set",
        );
    ...
    await t.test(
      "quoted local globs deduplicate matches and exclude hidden files unless explicit",
      async () => {
        ...
        const visible = await uploadJson([
          "up",
          path.join(globRoot, "**", "*.txt"),
          path.join(globRoot, "a.txt"),
          "--tag",
          "glob-visible",
        ]);
        assert.equal(
          visible.length,
          3,
          "a.txt should be deduplicated and hidden files excluded",
        );
        assert.deepEqual(visible.map((item) => item.name).sort(), [
          "a.txt",
          "b.txt",
          "c.txt",
        ]);
        const hidden = await uploadJson([
          "up",
          path.join(globRoot, ".*.txt"),
          "--tag",
          "glob-hidden",
    ...
    await t.test(
      "recursive folder uploads archive without following symlinks and extract safely",
      async () => {
        ...
        await writeFile(
          path.join(folder, "nested", "value.bin"),
          Buffer.from([9, 8, 7, 0]),
        );
        await symlink("nested/value.bin", path.join(folder, "value-link"));

        const [archive] = await uploadJson([
          "up",
          "-r",
          folder,
          "--tag",
        ...
        assert.equal(
          await readFile(path.join(extraction, "root.txt"), "utf8"),
          "root contents",
        );
        assert.deepEqual(
          await readFile(path.join(extraction, "nested", "value.bin")),
          Buffer.from([9, 8, 7, 0]),
        );
        assert((await lstat(path.join(extraction, "empty-dir"))).isDirectory());
        assert(
          (await lstat(path.join(extraction, "value-link"))).isSymbolicLink(),
        );
        assert.equal(
          await readlink(path.join(extraction, "value-link")),
          "nested/value.bin",
        );
      },
    );
    ...
    await t.test(
      "archive extraction rejects traversal before writing outside the destination",
      async () => {
        const maliciousArchive = gzipSync(
          singleFileTar("../escaped.txt", "should never be written"),
        );
        const response = await request(
          "/api/files?name=malicious.tar.gz&archive=tar.gz",
          {
            method: "POST",
            headers: {
    ...
    await t.test(
      "stdin upload and stdout download keep data channels byte-pure",
      async () => {
        const uploaded = await cli(
          ["up", "-", "--name", "stdin.bin", "--tag", "stdin", "--json"],
          { input: stdinBytes },
        );
        ...
        assert.equal(
          downloaded.stderr.length,
          0,
          "successful binary stdout must not be contaminated by diagnostics",
        );
      },
    );

    await t.test(
      "rm requires noninteractive confirmation and deletes only with --yes",
      async () => {
        ...
        assert.equal(
          (await request(`/raw/${stdinFile.id}`, {}, false)).status,
          404,
        );
    ...
    await t.test(
      "CLI maps usage, auth, not-found, conflict, network, and partial failures",
      async () => {
        ...
        const network = await cli(["list", "--json"], {
        ...
        assert(
          partialBody.some((item) => item.id === persistenceId && !item.error),
        );
    ...
    await t.test(
      "over-1-GiB sparse upload stops at noninteractive preflight without a request",
      async () => {
        const before = parseJson(
          await cli(["list", "--json"]),
          "list before large preflight",
        );
        ...
        const result = await cli(["up", sparsePath, "--no-input", "--json"], {
          timeout: 10_000,
        });
        ...
        assert.match(
          ...
          /continue unblocked work first/i,
        );
        assert.equal(result.stdout.length, 0);
        const after = parseJson(
          await cli(["list", "--json"]),
          "list after large preflight",
        );
        assert.equal(
          after.length,
          before.length,
          "rejected large upload must not reach server storage",
        );
    ...
    await t.test(
      "metadata and object bytes persist across a real server restart",
      async () => {
        ...
        const info = parseJson(
          await cli(["info", persistenceId, "--json"]),
          "info after restart",
        );
