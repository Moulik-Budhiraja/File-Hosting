...
docker compose exec -T server node -e 'const s=require("node:fs").readFileSync("/proc/self/status","utf8");const e=s.match(/^CapEff:\s*([0-9a-f]+)$/mi)?.[1];const b=s.match(/^CapBnd:\s*([0-9a-f]+)$/mi)?.[1];if(!e||!/^0+$/.test(e)||!b||BigInt(`0x${b}`)!==0x200000n)process.exit(1)'
