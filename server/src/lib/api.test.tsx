...
test("apiFetch exposes a valid Retry-After window", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn(
      async () =>
  ...
  const failure = await apiFetch("/api/auth/login").catch(
    (error: unknown) => error,
  );
  expect(failure).toBeInstanceOf(ApiError);
  expect((failure as ApiError).retryAfterSeconds).toBe(272);
});
