...
export function AuthProvider({
  ...
  const refreshWithMode = useCallback(
    async (probe: boolean) => {
      // Collapse concurrent refresh triggers (focus + storage + 403 can all
      // fire together) into one request. Genuine activity arriving during a
      // probe waits for it, then performs its own sliding refresh.
      const current = inflightRef.current;
      if (current) {
        if (probe || !current.probe) return current.promise;
      ...
      const entry = { probe, promise: task };
  ...
  const refresh = useCallback(() => refreshWithMode(false), [refreshWithMode]);

  const probe = useCallback(() => refreshWithMode(true), [refreshWithMode]);
