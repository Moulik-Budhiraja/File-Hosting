...
test("admins transfer ownership to an active user through the inspector", async () => {
  ...
  await userEvent.selectOptions(
    within(dialog).getByLabelText("Owner"),
    "u-admin",
  );
  await userEvent.click(
  ...
  await waitFor(() =>
    expect(
      record.some(
        (entry) =>
          entry.method === "PATCH" &&
          (entry.body as { owner_id?: string }).owner_id === "u-admin",
      ),
    ).toBe(true),
  );
  expect(
    screen.queryByRole("dialog", { name: "Transfer ownership" }),
  ).toBeNull();
});
...
test("empty state distinguishes an empty collection from filtered results", async () => {
  ...
  expect(screen.queryByText("No files match filters")).toBeNull();
...
test.each([
  ...
  [400, "invalid_owner", "Owner is required.", "Owner is required."],
  [
    403,
    "forbidden",
  ...
  "ownership transfer %i %s errors are terminal sentences without double punctuation",
  async (status, code, message, expected) => {
    ...
    expect(dialog.textContent).not.toContain(`${expected}.`);
  },
);

test("a valid past-the-end file cursor keeps recovery visible and restores page one", async () => {
  window.history.replaceState(null, "", "/files?cursor=stale-valid");
  vi.stubGlobal(
    "fetch",
    vi.fn(
      routes({
        files: (url) =>
          url.searchParams.has("cursor")
            ...
            : json(200, { items: [publicFile], next_cursor: null }),
      }),
    ),
  );
  ...
  expect(status.textContent).toBe("This page is empty.");
...
test("upload posts the file with the chosen visibility explained in plain language", async () => {
  ...
  expect(within(dialog).getByText("Only you and admins")).toBeTruthy();
...
test("an unverifiable visibility change reports an unknown outcome with retry", async () => {
  ...
  expect(await screen.findByText(/may or may not be saved/i)).toBeTruthy();
  expect(screen.queryByText(/nothing was changed/i)).toBeNull();
