...
test("throttled sign-in states the lock in words and disables the submit", async () => {
  ...
  expect(screen.getByText("Try again in 4 min 32 s.")).toBeTruthy();
...
test("correct credentials for a disabled account show recovery without username enumeration", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn(async () =>
      jsonResponse(403, {
  ...
  render(<LoginForm onSuccess={vi.fn()} />);
  await userEvent.type(screen.getByLabelText("Username"), "former-member");
  await userEvent.type(screen.getByLabelText("Password"), "correct password");
  await userEvent.click(screen.getByRole("button", { name: "Sign in" }));
  expect(screen.getByText("This account is disabled.")).toBeTruthy();
  expect(screen.getByText("Contact an administrator.")).toBeTruthy();
  expect(screen.queryByText(/username or password is incorrect/i)).toBeNull();
});

test("expired temporary password shows terse recovery for the deliberate server code", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn(async () =>
      jsonResponse(401, {
  ...
  render(<LoginForm onSuccess={vi.fn()} />);
  await userEvent.type(screen.getByLabelText("Username"), "invited-member");
  ...
  expect(screen.getByText("Temporary password expired.")).toBeTruthy();
  expect(screen.getByText("Ask an administrator for a new one.")).toBeTruthy();
  expect(screen.queryByText(/username or password is incorrect/i)).toBeNull();
});
...
test("an unreachable server yields a truthful unknown outcome, never an absolute claim", async () => {
  ...
  expect(screen.getByText(/sign-in outcome unknown/i)).toBeTruthy();
  expect(screen.queryByText(/nothing was changed/i)).toBeNull();
  expect(
    (screen.getByRole("button", { name: "Sign in" }) as HTMLButtonElement)
      .disabled,
