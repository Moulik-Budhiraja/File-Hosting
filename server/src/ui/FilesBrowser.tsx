...
export function FilesBrowser() {
  ...
  const [ownerOptions, setOwnerOptions] = useState<PublicUser[]>([]);
  ...
  useEffect(() => {
    ...
    void apiFetch<{ users: PublicUser[] }>("/api/users")
      .then(({ users }) => {
        if (cancelled) return;
        setOwnerOptions(users.filter((entry) => entry.active));
  ...
  async function saveVisibility() {
    ...
    try {
      ...
      if (isApiError(error) && error.status === 404) {
        ...
        setEditorError(
          "No confirmation. The change may or may not be saved. Retry or reload to check.",
        );
      }
    } finally {
      setBusy(false);
  ...
  async function saveOwner() {
    ...
    try {
      const updated = await apiFetch<FileMetadata>(
        `/api/files/${encodeURIComponent(selected.id)}`,
        { method: "PATCH", body: { owner_id: ownerValue } },
      );
      applyUpdated(updated);
    } catch (error) {
      if (isApiError(error) && error.status < 500) {
        ...
        try {
          ...
          if (current.owner_id === ownerValue) {
            applyUpdated(current);
            return;
          }
        } catch {
          // The record could not confirm the outcome.
        }
        setOwnerError("Transfer outcome unknown. Reload to check.");
      }
    } finally {
      setBusy(false);
    }
  ...
  return (
    <section aria-label="Files">
      ...
      {state.kind === "ready" && items.length === 0 && cursor ? (
        <div className="table-fallback">
          <p className="muted" role="status">
            This page is empty.
          </p>
            ...
            <tbody>
              {items.map((file) => (
                ...
                >
                  ...
                  <td className="cell-mono cell-muted cell-token col-desktop">
                    {file.mime_type}
                  </td>
                  <td
                    className="cell-mono cell-owner col-desktop"
                    title={ownerLabel(file)}
                  >
                    {ownerLabel(file)}
                  </td>
                  <td className="cell-mono cell-token col-desktop">
                    {file.visibility}
                  </td>
                  <td className="cell-mono cell-num col-desktop">
                  ...
                  <dd>
                    ...
                    {isAdmin ? (
                      <button
                        ...
                        onClick={() => {
                          setOwnerError(null);
                          setOwnerValue(selected.owner_id ?? "");
                          setOwnerOpen(true);
                        }}
                      >
                        Transfer…
      ...
      {ownerOpen && selected ? (
        ...
        >
          <div className="field">
            <label htmlFor={`${nameId}-owner`}>Owner</label>
            <select
              id={`${nameId}-owner`}
              value={ownerValue}
          ...
          <p>The new owner can manage this file immediately.</p>
          {ownerError ? (
            <p className="field-error" role="alert">
              {ownerError}
            </p>
          ...
          <div className="dialog-actions">
            ...
            <button
              type="button"
              className="button button-primary"
              disabled={busy || !ownerValue || ownerValue === selected.owner_id}
