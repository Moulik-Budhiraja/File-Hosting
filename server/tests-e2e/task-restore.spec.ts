...
async function relogin(
  ...
  // Preserve the legacy synthetic client header without granting it trust. The
  // production route ignores it unless the separate ingress proof is configured;
  // using a context header also avoids intercepting the reauth navigation.
...
test("an interrupted show-once pending flow reconciles truthfully after reauth", async ({
  ...
  await expect(dialog).toBeVisible();
  await expect(dialog.getByText("Secret unavailable.")).toBeVisible();
...
test("valid past-the-end Keys and Files cursors keep mobile recovery and restore page one", async ({
  context,
  page,
  baseURL,
}) => {
              ...
              api_keys: [
                {
                  ...
                  owner_username: "browser-owner",
                  name: "restored-browser-key",
                  prefix: "fsk_EXAMPLE",
                  last_four: "0000",
                  status: "active",
  ...
  await page.goto("/keys?cursor=valid-past-end");
  await expect(
    page.getByText(/This page is empty — 14 keys total/),
  ).toBeVisible();
  await expect(page.getByText("No API keys")).toHaveCount(0);
  await expect(page).toHaveURL(/cursor=valid-past-end/);
  const keysRecovery = page.getByRole("button", { name: "Back to first page" });
  await expect(keysRecovery).toBeVisible();
  const keyBox = await keysRecovery.boundingBox();
  expect(keyBox?.height).toBeGreaterThanOrEqual(44);
  expect(keyBox?.width).toBeGreaterThanOrEqual(44);
  await keysRecovery.click();
  await expect(page).not.toHaveURL(/cursor=/);
  await expect(
    page.getByRole("cell", { name: "restored-browser-key", exact: true }),
  ).toBeVisible();
        ...
        url.searchParams.has("cursor")
          ...
          : {
              items: [
                {
                  id: "browser-file",
                  name: "restored-browser-file.txt",
                  size: 12,
                  ...
                  tags: [],
                  preview_url: `${baseURL}/browser-file`,
                  raw_url: `${baseURL}/raw/browser-file`,
                  archive: null,
                  created_at: "2026-08-04T00:00:00.000Z",
  ...
  await page.goto("/files?cursor=valid-past-end");
  await expect(page.getByText("This page is empty.")).toBeVisible();
  await expect(page.getByText("No files")).toHaveCount(0);
  await expect(page).toHaveURL(/cursor=valid-past-end/);
  const filesRecovery = page.getByRole("button", {
    name: "Back to first page",
  });
  await expect(filesRecovery).toBeVisible();
  const fileBox = await filesRecovery.boundingBox();
  expect(fileBox?.height).toBeGreaterThanOrEqual(44);
  expect(fileBox?.width).toBeGreaterThanOrEqual(44);
  await filesRecovery.click();
  await expect(page).not.toHaveURL(/cursor=/);
  await expect(
    page.getByRole("button", { name: /restored-browser-file\.txt/ }),
  ).toBeVisible();
});
