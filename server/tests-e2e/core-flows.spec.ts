...
test("anonymous protected/private/missing previews are byte-identical branded unavailable pages", async ({
  request,
  baseURL,
}) => {
...
test("the private-safe not-found page signs in back to the original destination", async ({
  page,
}) => {
  await page.goto("/missing-recovery-destination");
  await page.getByRole("link", { name: "Sign in" }).click();
  await expect(page).toHaveURL(/\/login\?next=referrer$/u);
  await page.getByLabel("Username").fill("e2e-admin");
  await page.getByLabel("Password").fill("e2e-admin-password-longer-than-12");
  await page.getByRole("button", { name: "Sign in" }).click();
  await expect(page).toHaveURL(/\/missing-recovery-destination$/u, {
    timeout: 10_000,
  });
  await expect(
    page.getByRole("heading", {
      name: "This link doesn't exist or you don't have access.",
    }),
  ).toBeVisible();
});
